...
class TransactionController(TransactionConstants):
    ...
    def create_transaction(self, user_name: str, description: str, amount: int, type: bool, currency: str) -> bool:
        ...
        if type == self.DEBIT and amount > user_balance + currency_object.overdraft_limit:
            raise Exception(
                'Amount exceeds account balance and overdraft limit')
        ...
        self.check_triggers(amount)

        return True
    
    def check_triggers(self, amount):
        triggers = Trigger().getAll()
        
        for trigger in triggers:
            if amount > trigger.amount:
                print(f'########## TRIGGER ALERT {trigger.name} for amount exceeding {trigger.amount} ##########')